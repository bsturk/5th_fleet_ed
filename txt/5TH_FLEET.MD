...
|--------|------|---------|-------------|
| 0-1 | 0 | `(owner_raw << 8) \| template_id` | **Word 0**: Low byte = template index into TRMAIR/TRMSRF/TRMSUB.DAT, High byte = owner flags (bits 0-1 = side: 0=Green-A, 1=Green-B, 2=Red-A, 3=Red-B; see note below) |
| 2-3 | 1 | `position_slot` | **Word 1**: When `< region_count`, it is the on-map region id. Otherwise it encodes a slot ID that must be resolved through pointer section 14 (see below). |
...
#### Pointer Section 14: Unit Position Table

Pointer entry **14** in every scenario `.DAT` file contains a packed table that maps the 16-bit `position_slot` values back to actual spawn data. Each record is **11 bytes** long:

| Offset | Size | Meaning |
|--------|------|---------|
| 0x00 | `uint16` | `slot_start` — inclusive lower bound |
| 0x02 | `uint16` | `slot_end` — inclusive upper bound |
| 0x04 | `uint16` | `tile_x_raw` — usually multiples of 0x100 (divide by 256 for hex column) |
| 0x06 | `uint8`  | `tile_y_raw` |
| 0x07 | `uint8`  | `panel` (0/1 strategic panel, ≥2 tactical board) |
| 0x08 | `uint8`  | `flags` (bitfield/signature used by the loader) |
| 0x09 | `uint16` | `region_code` — low byte is the region id when `< 0x16`, otherwise a special off-map/staging code |

During loading the game performs a binary search across this table (`sub_50465` → `sub_A6088`) comparing the unit’s `position_slot` against `slot_start/slot_end`. Once a record is found, the panel/flag/tile metadata is copied to the runtime unit structure and the low byte of `region_code` becomes the effective region id (when valid). If no record matches, the engine raises the “Unit %s was not found in the Unit Position Table.” warning.

**Practical decoding rules:**
- If `position_slot < region_count`, treat it as a direct region index and display `tile_x` / `tile_y` straight from the 32-byte unit frame.
- Otherwise, look up the slot in pointer section 14 and:
  - Use `region_code & 0xFF` when it falls inside `[0, region_count)`.
  - Use `tile_x_raw >> 8` and `tile_y_raw` as the best available hex coordinates (star the value in the UI to indicate it was derived).
  - Surface `panel`/`flags` for debugging; non-zero panels indicate tactical submaps.
- Entries where `region_code & 0xFF == 0xFF` act as sentinels (fallback/no data).

#### Scenario Editor OOB Display

The editor now resolves `position_slot` values through this pointer section automatically. Slots that still live on the main board (values `< region_count`) retain their original region/coordinate fields, while reinforcement pools and scripted deployments show a derived region label plus a “*” suffixed coordinate that came from the Unit Position Table. The “Position:” line in the Unit Details box explains which entry was used (range, panel, flags) so you can verify the mapping against the disassembly references at `disasm.txt:295950-296196`.

**Note on owner_raw Encoding:**

**5th Fleet is a two-player wargame** (Green player vs. Red player), but uses 4 "side" values in the data format:
- **Bits 0-1**: Side encoding (0=Green-A, 1=Green-B, 2=Red-A, 3=Red-B)
  - Sides 0-1: Green player (US/Allied forces)
  - Sides 2-3: Red player (Soviet/Russian/Indian forces)
  - Analysis suggests **bit 1 determines player ownership** (0=Green, 1=Red), while **bit 0 may indicate formation/sub-group**
- **Higher bits**: Mission state flags (e.g., bombing assignment) as seen in `disasm.txt:188498+`

All 4 side values are actively used in scenario data. The "A/B" designation likely represents primary/allied forces or different formation groups within each player's forces.
...
- **Victory points per template:** the data that feeds the in-game “Victory Pt:” readouts lives inside the same records.
  - Air templates store VP as the **last non-zero byte** of each record (single-byte value; e.g., `AV-8B` → `0x0a` = 10 VP, `B-52G` → `0x47` = 71 VP).
  - Surface and submarine templates store VP as a **little-endian word at offset `0x72`** (e.g., `Adams` → `0x001e` = 30 VP, `Los Angeles` → `0x0012` = 18 VP).
  - These offsets match the structures accessed by the `Victory Pt:` UI routines in `sub_B1357`/`sub_B2048`, confirming they are the tables referenced by the `SCORE` opcode.
...
- Graphics/UI assets:
  ...
  - Map counter art: tactical map icons (26×26 pixels) live in `MICONRES.RES` as 66 `MICN` records (MICN = "Map ICon"). Each record has a 16-byte header followed by pixel data:
    ...
    - The background color index is replaced by the engine with side-specific colors (0=green, 1=green, 2=red, 3=red for player ownership; actual display colors may vary) to indicate unit ownership
...
- **Current Scenario Editor Coverage Gaps**
  - `SHIP_DEST` port records are read-only: every map stores 70-byte port entries that embed `fb 06` markers for objective destinations (`txt/5TH_FLEET.MD`:491-519), yet the Tk editor only exposes region metadata (`scenario_editor.py`:342-415). Convoy scenarios described in `scenarios.md` (e.g., #2 Russian Raiders, #4 Carrier Raid) therefore cannot be retargeted from the UI.
  - Pointer section 6 (convoy port lists) is never loaded. The editor’s loader only treats pointer indices 5/8/11 as unit tables (`editor/data.py`:10-20) even though `CONVOY_FALLBACK`/`PORT_LIST` opcodes pull their destinations from section 6 (`txt/5TH_FLEET.MD`:973-975). Objectives such as “Aden, Bandar ’Abbas, Bombay, or Cochin” in Scenario 5 remain locked to the original binary.
  - Pointer sections 0 and 1 (task-force/unit lookup tables) stay opaque. Opcodes like `TASK_FORCE`, `SHIP_OBJECTIVE`, `TASK_GROUP`, etc. reference these tables (`txt/5TH_FLEET.MD`:973-974), but the editor only adds lookup helpers for sections 9 and 14 (`scenario_editor.py`:1426-1684). Named targets from `scenarios.md` (e.g., “Sink the Ushakov”) cannot be reassigned.
  - Section 12 deployment scripts are untouched. The engine programs reinforcements via `(opcode, operand)` tuples stored in pointer section 12 (`txt/5TH_FLEET.MD`:976, `disasm.txt`:189182-189200), yet the OOB tab filters out every unit whose `region_index` ≠ 0 (`scenario_editor.py`:1218-1223). Reinforcement waves and stacked formations described in the rulebook cannot be edited.
  - Unit metadata (names/classifications) in pointer section 14 cannot be modified. Investigation shows strings like “Antares”/“Capella” plus their “Fast Convoy” tags are stored there (`txt/OBJECTIVE_PARSING_INVESTIGATION_REPORT.md`:173-209), but `UnitRecord` only persists template/owner/region words (`editor/data.py`:457-475). Scenario narratives that rely on named ships (e.g., `scenarios.md`:5-22, 85-103) cannot be updated.
  - Objective port markers are parsed but never written. `_extract_objective_ports()` displays ports flagged with `SHIP_DEST(251)`, yet there is no UI to flip those markers before saving (`scenario_editor.py`:1540-1594), so even detected destinations remain uneditable.
  - Victory-point tables referenced by the `SCORE` opcode remain hidden. The operand indexes a VP table sitting ahead of the opcode stream (`txt/OBJECTIVE_PARSING_INVESTIGATION_REPORT.md`:137-170), and `_encode_objective_script()` explicitly preserves that metadata blob (`scenario_editor.py`:1929-1950). Users can swap opcodes but not the thresholds that enforce “destroy as many enemy units as possible”.
  - Logistics/campaign masks are still “magic numbers”. Opcodes such as `PORT_RESTRICT`, `SUPPLY_LIMIT`, and `SCENARIO_INIT_xx` only receive terse descriptions (`scenario_editor.py`:1801-1919) even though they expect bitmasks referencing port tables. Supply restrictions called out in `scenarios.md` (e.g., Scenario 6’s convoy logistics) cannot be authoritatively edited.
    ...
    "sections": [
      {
        ...
        "preview": {
          ...
          "side_counts": [[0, 16], [1, 4], [2, 6], [3, 4]],  # 0-1=Green player, 2-3=Red player
